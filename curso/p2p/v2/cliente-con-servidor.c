...
int main(int argc, char *argv[])
{
    ...
    //A RELLENAR
    //Contactar con el servidor y mandar el mensaje de que queremos registrarnos
    //Comprobar respuesta: si positiva, continuamos, si no, paramos
...
void leer_y_procesar_teclado(int socketUDP)
{
    ...
    if (linea[0] == '/')
    {
        ...
        if (strcmp(cmd, "/CHAT") == 0)
        {
            ...
            //A RELLENAR
            //Cambia sscanf (tiene 2 especificadores de formato)
            //1 Contactar con el servidor (crear conexion tcp) y enviar QUERY cmd (preguntar por la longitud del nick)
            //2.1 Si no lo encontró: imprimir no existe o cambiar nick
            //2.2 Si lo encontró: Los campos de la estructura dir_destino hay que rellenarlos con el endpoint que devuelve el servidor
        ...
        else if(strcmp(cmd, "/QUIT") == 0)
        {
            close(socketUDP);
            //A RELLENAR
            //LEAVE cmd (desregistrarse)
            exit(0);
        }
        else
        {
            printf("Error: El comando es desconocido.\n");
            return;
        }
    }
