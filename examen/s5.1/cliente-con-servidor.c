...
char *ip_servidor; int puerto_servidor;
char respuesta;
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 5)
    {
        printf("Uso: %s puerto nick ip_servidor puerto_servidor\n", argv[0]);
        exit(0);
    }
    nick = argv[2];
    ...
    resultado = Conectar(socketTCP, ip_servidor, puerto_servidor);

    sendByte(socketTCP, JOIN_CMD);
    sendShort(socketTCP, strlen(nick));
    sendString(socketTCP, nick);
    sendShort(socketTCP, puerto);
    respuesta = readByte(socketTCP);
    if (respuesta == CMD_OK)
        printf("Nick registrado\n");
    else
        printf("Nick no registrado\n");
...
void leer_y_procesar_teclado(int socketUDP)
{
    ...
    if (linea[0] == '/')
    {
        ...
        if (strcmp(cmd, "/CHAT") == 0)
        {
            ...
            if (sscanf(linea, "%s", nick) != 1)
            {
                printf("Uso correcto del comando: /CHAT <nick>\n");
                return;
            }

            socketTCP = CrearSocketDatosTCP();
            Conectar(socketTCP, ip_servidor, puerto_servidor);

            sendByte(socketTCP, QUERY_CMD);
            sendShort(socketTCP, strlen(nick_amigo));
            sendString(socketTCP, nick_amigo);
            respuesta = readByte(socketTCP);
            if (respuesta == QUERY_ERR) {
                printf("El usuario %s no se encuentra", nick_amigo);
            }
            else {
                readEndpoint(socketTCP, &dir_destino);

                dir_destino.sin_port = htons(puerto_destino);
                puerto_destino = ntohs(dir_destino.sin_port);
                strcpy(ip_destino,inet_ntoa(dir_destino.sin_addr));
        ...
        else if (strcmp(cmd, "/QUIT") == 0)
        {
            ...
            sendByte(socketTCP, LEAVE_CMD);
            sendShort(socketTCP, strlen(nick));
            sendString(socketTCP, nick);
            respuesta = readByte(socketTCP);
            if (respuesta == LEAVE_ERR)
                printf("El usuario no estaba previamente registrado");
            else printf("Usuario desregistrado con éxito");
            exit(0);
        }
        else
        {
