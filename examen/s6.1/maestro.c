...
// hace la operación correspondiente sobre la posición del array de mutex
// y envía confirmación
void OperarMutex(petinfo *pet, int socket_datos, int *lista, int listalen);

// Esta función inicializa cada posición del array de mutex

void InicializaMutex(pthread_mutex_t *lista, int tam);

// Procesa una nueva petición de acceso a un recurso
void *procesaPeticion(petinfo *p);

// Programa principal.
// Crea un socket de escucha y queda a la espera de un cliente.
// Cuando llega una nueva petición, genera un nuevo hilo
// que realmente es el que atiende la petición
int main(int argc, char *argv[])
{
    ...
    int puerto;
    // número de mutex que protegen los recursos, uno por cada recursos
    int num_mutex;
    ...
    {
        fprintf(stderr, "Forma de uso: número de puerto no válido.\n");
        exit(2);
    }
    num_mutex = atoi(argv[2]);
    if (num_mutex < 0)
    {
        fprintf(stderr, "Forma de uso: el número de mutex debe ser positivo.\n");
        exit(3);
    }

    /**************************************************************/
    // Informar al usuario del número de puerto que se usará
    printf("Máster recursos: usando el puerto = % d\n", puerto);
    ...
    while (1)
    {
        ...
        // almacenamos en los campos de la estructura a la que apunta la info
...
// función a ejecutar por cada hilo que procesa una petición de acceso
// a un recurso
